...
exports.addMenu = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    if (err.code == 11000) {
      err.errmsg = req.i18n.__("RESTAURANT_UNIQUE_ERROR");
    }
    return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': (err.errmsg) ? (err.errmsg) : err });
...
exports.Getlocation = async (req, res) => {
  try {
    ...
    console.log("req.params.longitude", parseFloat(req.params.longitude).toFixed(6));
    ...
    let getRestaurant = await RestaurantModel.find({ restaurantLocation: { $near: { $geometry: { type: "Point", coordinates: [parseFloat(req.params.longitude).toFixed(6), parseFloat(req.params.latitude).toFixed(6)] }, $maxDistance: Config.RadiusLimit } } })
      .populate('cuisineId', 'cuisineName _id')
      .populate({ path: 'cuisineId', match: { 'status': _C.status.adminPanel.active }, select: 'cuisineName -_id' })
    ...
    if (getRestaurant && getRestaurant.length > 0) {
      ...
      let Restaurant = _.map(filterDoc, function (o) {
        //if(!(o.restaurantTimingId.hours==undefined)){
        ...
        //}throw { errmsg: "INVALID_TIMING_ID" }
        ...
        var getDay2 = _.map(getDayCount, function (day) {
          return day.openingLabel
        }).join(",")
        var closedday = weekday[now.getDay() + 1]
        let closedday2 = (closedday == 'monday') ? o.restaurantTimingId.hours.monday : (closedday == 'tuesday') ? o.restaurantTimingId.hours.tuesday : (closedday == 'wednesday') ? o.restaurantTimingId.hours.wednesday : (closedday == 'thursday') ? o.restaurantTimingId.hours.thursday : (closedday == 'friday') ? o.restaurantTimingId.hours.friday : (closedday == 'saturday') ? o.restaurantTimingId.hours.saturday : o.restaurantTimingId.hours.sunday;
        var deal = _.filter(closedday2, function (day) {
          return ((day.opening <= seconds) && (day.closing >= seconds));
        });
        var opens = _.map(deal, function (day) {
          return day.openingLabel
        }).join(",")
        return {
          _id: o._id,
          ...
          nextOpening: (getDayCount.length !== 0) ? getDay2 : opens,
        }
      })      
