...
exports.getCustomerReport = async function (req, res) {
	try {
		...
		if (req.query.fromDate && req.query.toDate) {
			cond['orderDate'] = { $gte: new Date(moment(req.query.fromDate).startOf('day').format('YYYY-MM-DD')), $lte: new Date(moment(req.query.toDate).endOf('day').format('YYYY-MM-DD')) };
		} else if (req.query.fromDate) {
			console.log("2222", req.query.fromDate);
			cond['orderDate'] = { $gte: new Date(moment(req.query.fromDate).startOf('day').format('YYYY-MM-DD')) };
		} else if (req.query.toDate) {
			console.log("33333", req.query.toDate);
			cond['orderDate'] = { $lte: new Date(moment(req.query.toDate).endOf('day').format('YYYY-MM-DD')) };
			console.log(moment(req.query.toDate).endOf('day').format(('YYYY-MM-DD')))
		}
