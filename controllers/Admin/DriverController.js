    ...
    try {
        ...
        let userUpdate = await DriverModel.findOneAndUpdate({
        ...
        },
            {
                ...
                'profileImage': (profileImage1) ? profileImage1 : `http://${Config.server.host}:${Config.server.port}/user/default.png`
            },
        ...
        if (userUpdate && userUpdate.length !== 0) {
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("DRIVER_DATA_UPDATE_SUCCESS"), 'data': userUpdate });
        }
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        let condition = { _id: req.params.id, status: { $in: [_C.status.driver.active, _C.status.driver.offline, _C.status.driver.online] } };
        let deleteUser = await DriverModel.findOneAndUpdate(condition, { status: _C.status.driver.deleted }, { new: true });
...
exports.list = async (req, res) => {
    ...
    try {
        ...
        if (req.params.id) {
            pagination = false;
            condition = { _id: req.params.id, status: { $ne: [_C.status.driver.deleted, _C.status.driver.inactive] } };
        }
        if (pagination) {
            condition = likeQuery;
        ...
        if (getDriverList && getDriverList.length > 0) {
            var user = _.map(getDriverList, function (o) {
                return {
                    _id: o._id,
                    userName: o.userName,
                    mobile: o.mobile,
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("DRIVER_LIST_SUCCESS"), 'data': user });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("DRIVER_NOT_FOUND") });
    } catch (err) {
...
exports.getdriver = async (req, res) => {
    try {
        ...
        if (req.params.id) {
            condition = { _id: req.params.id, status: { $in: [_C.status.driver.active, _C.status.driver.offline, _C.status.driver.online] } };
        }
