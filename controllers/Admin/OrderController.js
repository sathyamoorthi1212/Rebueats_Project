...
exports.listOrder = async (req, res) => {
    try {
        ...
        var getvalue = _.map(getOrderList, function (o) {
            console.log(o);
            return {
                userName: (o.userId.firstName) + (o.userId.lastName),
                ...
                restaurant_address: o.restaurantId.address ? o.restaurantId.address : "null"
            }
        })
        if (getOrderList && getOrderList.length > 0) {
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("ORDER_LIST_SUCCESS"), 'data': getvalue });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("ORDER_NOT_FOUND"), 'data': [] });
    } catch (err) {
...
exports.acceptedGenerateCSV = async (req, res) => {
    try {
        ...
        if (getAccepted && getAccepted.length > 0) {
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("ORDER_ACCEPTED_LIST_SUCCESS"), 'data': getvalue });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("ORDER_ACCEPTED_NOT_FOUND"), 'data': {} });
    } catch (err) {
        console.log(err);
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': (err.errmsg) ? (err.errmsg) : err });
    }
...
exports.pendingGenerateCSV = async (req, res) => {
    try {
        ...
        if (getOrder && getOrder.length > 0) {
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("ORDER_PENDING_LIST_SUCCESS"), 'data': getvalue });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("ORDER_PENDING_NOT_FOUND"), 'data': {} });
    } catch (err) {
        console.log(err);
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': (err.errmsg) ? (err.errmsg) : err });
    }
    ...
    try {
        ...
        var getvalue = _.map(getDeliveryList, function (o) {
            ...
            return {
                ...
                paymentType: (o.paymentType == 1) ? "cash" : "online",
                paymentReceived: (o.paymentReceived == 1) ? "received" : "Not received",
                orderStatus: (o.orderStatus.code == 7) ? "Delivery" : "",
                orderDate: moment(o.orderDate).format("MMMM Do YYYY, h:mm:ss a"),
                restaurantName: o.restaurantId.restaurantName,
                restaurant_mobile: o.restaurantId.mobile,
                restaurant_email: o.restaurantId.email,
...
exports.getDelivery = async (req, res) => {
    try {
        ...
        if (getDelivery && getDelivery.length > 0) {
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("DELIVERY_LIST_SUCCESS"), 'data': getvalue[0] });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("DELIVERY_NOT_FOUND"), 'data': {} });
    } catch (err) {
        console.log(err);
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': (err.errmsg) ? (err.errmsg) : err });
    }
...
exports.deliveryGenerateCSV = async (req, res) => {
    try {
        ...
        if (getOrder && getOrder.length > 0) {
            ...
            return res.status(HttpStatus.OK).json({ 'success': true, 'message': req.i18n.__("ORDER_DELIVERY_LIST_SUCCESS"), 'data': getvalue });
        }
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': req.i18n.__("ORDER_DELIVERY_NOT_FOUND"), 'data': {} });
    } catch (err) {
        console.log(err);
        return res.status(HttpStatus.NOT_FOUND).json({ 'success': false, 'message': (err.errmsg) ? (err.errmsg) : err });
    }
