    ...
    try {
        ...
        if (user && ((user.status !== 2) && (user.status !== 5))) {
            ...
            if (HashService.compareHash(user.password, req.body.password)) {
                ...
                let userData = await PartnerModel.findOneAndUpdate({ '_id': user._id }, { 'status': 3, 'loginAttempts': loginAttemptCount }, { new: true }).select('otpVerification');
...
exports.logout = async function (req, res) {
    ...
    try {
        ...
        let adminData = await PartnerModel.findOneAndUpdate(condition, { status: 4 }, { new: true }).select('-password');
