...
exports.listOrder = async (req, res) => {
    try {
        ...
        let getOrderList = await OrderModel.find(condition).sort(sortQuery).skip(pageQuery.skip).limit(pageQuery.take)
            .select('_id orderCode total paymentType orderDate orderStatus items.itemName items.totalPrice items.quantity items.price')
...
exports.getOrder = async (req, res) => {
    try {
        if (req.params.id) {
            condition = { '_id': req.params.id, 'status': _C.status.adminPanel.active, 'orderStatus.code': _C.status.order.pending };
        }
...
exports.getAccepted = async (req, res) => {
    try {
        if (req.params.id) {
            condition = { '_id': req.params.id, 'status': _C.status.adminPanel.active, 'orderStatus.code': _C.status.order.accepted };
            condition['orderStatus.acceptedAt'] = { $exists: true };
        }
