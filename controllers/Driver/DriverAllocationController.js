...
const DriverPaymentReportModel = require('../../models/DriverPaymentReport');
...
function updateData(datas) {
    ...
    var id      = datas.driverId.toString(); 
    var OrderId = datas.orderId.toString();      
    var update  = {};
    ...
    orderUpdate["driverId"]   = id;   
    db.ref("orders").child(OrderId).update(orderUpdate,function(err){
        if(err){}        
        ...
        if (order.orderStatus.code === _C.status.order.confirmed) {
            ...
            if (_drivers.length > 0) {
                ...
                _drivers.map(async driver => {
                    ...
                    req.body.createdRef = (req.body.createdRef == 1) ? 'Admin' : (req.body.createdRef == 2) ? 'Driver' : 'User';              
                    let allocatedRequest = await DriverAllocationModel.findOneAndUpdate(request, req.body.createdRef, { upsert: true, new: true });
                    console.log("allocatedRequest",allocatedRequest)
                    autoUpdateData(allocatedRequest.driverId,allocatedRequest.orderId)
                    if (allocatedRequest) {
...
function autoUpdateData(datas,orderId) {
  
    if (!firebase.apps.length) {
       firebase.initializeApp(Config.firebasekey);
    }
    var id      = datas.toString(); 
    ...
    try {
        ...
        if (booking.orderStatus.code === _C.status.order.confirmed) {
            ...
            if (allocatedUpdate != null) {
                /*Andriod side query dump issue so hide thi part*/
                ...
                acceptedUpdateData(orderUpdate)
                if (orderUpdate) {
...
function acceptedUpdateData(datas) {
  
    if (!firebase.apps.length) {
       firebase.initializeApp(Config.firebasekey);
    }
    var driverId      = datas.driverId.toString(); 
    var OrderId = datas._id.toString();      
    var update  = {};
    var db      = firebase.database();
    update["orderStatus"]     = datas.orderStatus.code;   
    update["driverId"]        = driverId;   
    db.ref("orders").child(OrderId).update(update,function(err){
        if(err){}        
        logger.info(update)
    }); 
...
exports.orderReject = async function(req, res) {
    ...
    try {
        ...
        if (booking.orderStatus.code === _C.status.order.confirmed) {
            ...
            console.log(allocatedUpdate)
            if (allocatedUpdate) {
                rejectUpdateData(allocatedUpdate);
...
function rejectUpdateData(datas) {
    ...
    var update  = {};
    var db      = firebase.database();
    update["orderStatus"]     = _C.status.order.accepted;   
    update["driverId"]        = driverId;   
    update["rejectedReason"]  = datas.rejectedReason;   
    db.ref("orders").child(OrderId).update(update,function(err){
        if(err){}        
        logger.info(update)
    }); 
...
exports.changeOrderStatus = async function(req, res) {
    ...
    try {
        ...
        if(booking){
               let driverPaymentReport = await DriverPaymentReportModel().addDriverPaymentReport(booking);
               //updateStatusData(booking);
...
function updateStatusData(datas) {
